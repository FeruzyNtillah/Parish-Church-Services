    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full md:w-1/2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white z-50 overflow-auto p-6 border-l border-gray-200 dark:border-gray-700 shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-2xl font-semibold text-gray-900 dark:text-white">{family.familyName}</h3>
            <div className="text-sm text-gray-500 dark:text-gray-400">Created: {family.createdAt}</div>
          </div>
            ...
            {familyMemberList.map((fm) => {
              ...
              return (
                <div key={fm.id} className="flex items-center justify-between p-3 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => onRemoveMember(fm.id)} className="p-2 rounded text-red-600 dark:text-red-400 border border-gray-300 dark:border-gray-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors" aria-label={`Remove ${m?.fullName} from family`} title="Remove member">
